...
def load_typecho_config():
    if os.path.exists(TYPECHO_CONFIG_FILE):
        try:
            with open(TYPECHO_CONFIG_FILE, 'r', encoding='utf-8') as f:
                return json.load(f)
        except Exception:
            pass
    return {
...
def save_typecho_config(config):
    with open(TYPECHO_CONFIG_FILE, 'w', encoding='utf-8') as f:
        json.dump(config, f, ensure_ascii=False, indent=2)

# 加载Alist账户
def load_alist_accounts():
    if os.path.exists(ALIST_CONFIG_FILE):
        try:
            with open(ALIST_CONFIG_FILE, 'r', encoding='utf-8') as f:
                return json.load(f)
        except Exception:
            pass
    return []
...
def save_alist_accounts(accounts):
    with open(ALIST_CONFIG_FILE, 'w', encoding='utf-8') as f:
